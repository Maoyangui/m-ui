...
// 停用的用户在浏览器里应看到"订阅不可用"这一页(带主面板的公告与联系方式),
// 客户端拿到的仍然是干净的 404。
func TestNotFoundPageForBrowser(t *testing.T) {
	...
	body := w.Body.String()
	for _, want := range []string{"订阅不可用", "每月 1 号重置流量", "TG: @support"} {
	...
	// 地址根本不存在时也给同一页(不暴露这个地址是否曾经存在)
	n := doReqZh(s, "/sub/nobody-here")
	if n.Code != 404 || !strings.Contains(n.Body.String(), "订阅不可用") {
		t.Fatalf("未知地址应给同一页,实际 %d", n.Code)
	}
}
...
func TestNotFoundPageUsesResellerCopy(t *testing.T) {
	...
	for _, want := range []string{"dl1 机场", "本店公告", "TG: @dl1"} {
		if !strings.Contains(body, want) {
