...
[ -n "$LOCAL_ID" ] && api -X PUT "$M/api/nodes/$LOCAL_ID" -d '{"name":"Frankfurt","domain":"panel.example.com","addr":"192.0.2.10","ratio":1,"sort":0}' >/dev/null
api -X POST "$M/api/nodes" -d "{\"name\":\"Tokyo\",\"domain\":\"tokyo.example.com\",\"addr\":\"198.51.100.10\",\"apiUrl\":\"http://127.0.0.1:19153/app/\",\"token\":\"$TOKEN\",\"insecure\":true,\"enabled\":true,\"ratio\":1,\"sort\":1}" >/dev/null
api -X POST "$M/api/nodes" -d "{\"name\":\"Singapore\",\"domain\":\"sg.example.com\",\"addr\":\"203.0.113.30\",\"apiUrl\":\"http://127.0.0.1:19253/app/\",\"token\":\"$TOKEN\",\"insecure\":true,\"enabled\":true,\"ratio\":2,\"sort\":2}" >/dev/null
...
line '{"name":"Hysteria2","protocol":"hysteria2","port":24443,"upstreamId":0,"enabled":true,"nodeIds":[],"tls":{"mode":"cert"},"options":{"up_mbps":0,"down_mbps":0},"assignAll":true}'
line '{"name":"AnyTLS","protocol":"anytls","port":28443,"upstreamId":0,"enabled":true,"nodeIds":[],"tls":{"mode":"cert"},"options":{},"assignAll":true}'
line "{\"name\":\"Reality\",\"protocol\":\"vless\",\"port\":24433,\"upstreamId\":0,\"enabled\":true,\"nodeIds\":[],\"tls\":{\"mode\":\"reality\",\"reality\":{\"handshake_server\":\"www.microsoft.com\",\"handshake_port\":443,\"private_key\":\"$PRIV\",\"public_key\":\"$PUB\",\"short_ids\":[\"$SID\"]}},\"options\":{\"vision\":true},\"assignAll\":true}"
line "{\"name\":\"Shadowsocks 2022\",\"protocol\":\"shadowsocks\",\"port\":28388,\"upstreamId\":1,\"enabled\":true,\"nodeIds\":[],\"options\":{\"method\":\"2022-blake3-aes-128-gcm\",\"password\":\"$SSPW\"},\"assignAll\":true}"

echo "用户"
NOW="$(date +%s)"
user() { api -X POST "$M/api/users" -d "$1" >/dev/null; }
user "{\"name\":\"demo-alice\",\"enabled\":true,\"remark\":\"Pro - monthly\",\"volume\":$((200*1024*1024*1024)),\"expiry\":$((NOW+30*86400)),\"deviceLimit\":3,\"autoReset\":true,\"resetDays\":30,\"lineIds\":[1,2,3,4]}"
...
user "{\"name\":\"demo-carol\",\"enabled\":true,\"remark\":\"Team\",\"volume\":$((500*1024*1024*1024)),\"expiry\":$((NOW+90*86400)),\"deviceLimit\":5,\"lineIds\":[1,2,3,4]}"
user "{\"name\":\"demo-dave\",\"enabled\":true,\"remark\":\"Trial\",\"volume\":$((20*1024*1024*1024)),\"expiry\":$((NOW+3*86400)),\"deviceLimit\":1,\"lineIds\":[1,4]}"
user "{\"name\":\"demo-eve\",\"enabled\":true,\"remark\":\"Unlimited\",\"volume\":0,\"expiry\":0,\"deviceLimit\":0,\"lineIds\":[1,2,3,4]}"
