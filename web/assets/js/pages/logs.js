...
import { esc, fmtDate, registerActions, empty, debounce } from '../ui.js';
...
async function renderTab() {
  ...
  if (tab === 'sub') {
    el.innerHTML = `
      <div class="toolbar"><input type="search" id="logs-user" placeholder="${t('logs.filterUser')}" value="${esc(userFilter)}"><button class="btn sm" data-act="logs.refresh">${t('common.refresh')}</button></div>
    ...
    el.innerHTML = `
      <div class="toolbar"><select class="sm" id="logs-level" style="width:auto">${['debug', 'info', 'warning', 'error'].map(l => `<option ${l === level ? 'selected' : ''}>${l}</option>`).join('')}</select><button class="btn sm" data-act="logs.refresh">${t('common.refresh')}</button></div>
    ...
    el.innerHTML = `
      <div class="toolbar"><button class="btn sm" data-act="logs.refresh">${t('common.refresh')}</button></div>
