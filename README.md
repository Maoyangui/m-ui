...
- 新建的代理有 24 小时"认领窗口":这段时间内用名字空密码登录一次并设好密码,过期未认领就得由主面板再点一次"重置密码"重开窗口,避免账号一直敞着。
...
**代理登录不上?** 新代理有 24 小时认领窗口:窗口内用代理名 + 空密码登录并设密码。过期或忘记密码,主面板代理页"更多 → 重置密码"清空后重开窗口(两步验证会一并关掉)。
