...
- 代理可以配自己的订阅落地页(标题、公告、客服),留空则继承主面板;也能单独关掉自己的订阅页与临时共享。
- 主面板的代理详情里能看到他的每个用户:订阅地址、用量、到期、在线 IP 与所连线路。
...
- **无域名**:用服务器 IP 生成自签证书。默认只给线路入站用,订阅链接自动带"允许不安全",客户端打开该开关即可连;面板与订阅保持 HTTP(自签走 HTTPS 会被浏览器和客户端拦下)。
...
**时区**(默认 Asia/Shanghai,面板里所有时间按它显示)、面板与订阅的监听地址 / 端口 / 路径 / 证书、订阅展示选项、落地页文案、Telegram 通知项、上游巡检参数、数据面统计粒度等。改面板路径 / 订阅路径保存即生效;改端口、监听地址、证书路径需重启,页头有重启按钮。
