...
| 代理 | 给代理分配线路与流量 / 设备额度,他在自己的面板(默认 2054,路径 /dl)里建号;用量与设备数汇总到代理,超额自动停其全部用户 |
...
- 代理在自己的面板里只看得到自己的用户:建号只填名称(订阅地址是一串随机令牌,link / Clash / sing-box 三种格式与二维码都照常),可分配线路(只限主面板给他的)、流量、到期、设备、限速、套餐,也能续费、重置、踢线、看用量曲线与在线设备。
- 额度:名下用户用量之和达到代理额度,他的用户全部停用(主面板可重置);代理分给用户的设备上限之和不得超过设备总额。
