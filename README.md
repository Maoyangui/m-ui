...
- **改配置不担心断线**:保存前先把整份配置交给 sing-box 干跑校验,坏配置进不了库;改用户和上游都是热更新,只有改线路才重启数据面。万一新配置校验通过却起不来(比如端口刚好被同机别的程序抢走),会自动回滚到上一份可用配置,不会把所有用户晾在那里。
...
| 订阅 | 通用链接、Clash、sing-box 三种格式;浏览器打开是落地页(用量、到期、一键导入、二维码、客户端下载);用户可自助生成临时共享地址;外部节点 / 外部订阅聚合;自签证书自动带 insecure |
...
一个进程里跑三件事:**面板**(给你用)、**订阅服务**(给客户端拉配置)、**数据面**(内嵌 sing-box,真正过流量)。三者共用同一个 SQLite 文件,改动即时生效。

```mermaid
flowchart LR
  A[管理员浏览器] -->|:2053 /app| P[面板 HTTP]
  R[代理浏览器] -->|:2054 /dl| D[代理面板<br/>同一套前端·会话带作用域]
  C[客户端<br/>Clash / sing-box / 小火箭] -->|:2056 /sub| S[订阅服务]
  C -->|连接节点| X[数据面<br/>内嵌 sing-box]
...
**改一次配置发生了什么**——面板从不直接改运行中的 sing-box:

```mermaid
flowchart LR
  E[保存线路 / 用户] --> V{整份配置<br/>交给 sing-box 干跑}
  V -- 不通过 --> K[拒绝保存<br/>旧配置继续服务]
  V -- 通过 --> W[(写库)]
  W --> H{改了什么}
  H -- 用户 --> HU[热换入站用户表<br/>不断开其他人]
  H -- 上游 --> HO[热换出站<br/>数据面不重启]
  H -- 线路 --> HR[重启数据面<br/>失败则回滚到上一份]
```

**多服务器**是一主多副:主机算账、推配置,副机只管转发。

```mermaid
flowchart LR
  M[主服务器<br/>面板 · 订阅 · 判定配额] -->|每 5s 推快照<br/>线路/上游/用户/凭据| N1[副机 A]
  M -->|同上| N2[副机 B]
  N1 -->|回传 流量增量 · 在线 IP| M
  N2 -->|回传| M
  M -->|下发 其它机器的在线 IP| N1
...
```mermaid
flowchart TB
  L[线路<br/>协议+端口 → 上游] --- UL{{用户 ↔ 线路}}
  UL --- U[用户<br/>配额·到期·设备·限速]
  U --> SUB[订阅<br/>link / clash / sing-box]
  L --> UP[上游<br/>落地出口]
  RS[代理<br/>额度容器 + 独立面板] -->|拥有| U
  RS -->|被授权| L
  PL[套餐<br/>模板] -.套用.-> U
```
...
| 组件 | 职责 | 代码 |
|---|---|---|
| 面板 | 管理界面与 API、代理面板、外部 API | `web/` |
| 渲染 | 线路 → sing-box 配置,保存前干跑校验 | `render/` `core/validate.go` |
| 数据面 | 内嵌 sing-box、流量统计、连接跟踪、限速与设备数 | `core/` |
| 订阅 | 三种格式、落地页、临时共享 | `sub/` |
| 同步 | 主副机快照推送、流量回收、在线聚合 | `hub/` |
| 编排 | 进程生命周期、证书、备份、外部节点 | `runner/` |
| 定时 | 统计、配额判定、清理 | `jobs/` `monitor/` |
